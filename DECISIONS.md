...
| Writing → Making | Variant A, "lift off the page" (owner, 2026-09-22). When the last word lands, a short beat shows both sentences whole; then the bar, prompt, guides and footnote fold away, the sentences stay where they are, and from a to z each letter lifts out of its word in full ink, flies to its slot and turns to metal type while the rest of the sentence goes faint. After z the sentences fold away and Making carries on as built. Reference: `design/done-transition.html` (A). Rejected: B, "hold to tear it off" (a hold on Done tears the page off a pad); C, "draw a line under it" | It delivers "your own letters as the actors: lifted off the page, cast into type". Before, the page faded out and the letters appeared in empty slots from nowhere |
| Done button | None (owner, 2026-09-22). Finishing the 15th word starts Making, with no tap, no confirm step and no hold | Watching your letters leave their words makes it obvious the page is done, so the button had nothing left to say |
...
| Logo is home | Past Hero, the logo is a button back to Hero. The writing stays in memory, so picking an ink returns to it with every word kept; a build in progress is dropped. Coming back to a finished page offers **Make my font** (see *Starting Making*) | Lets people get back without a reload, and nothing they wrote is lost |
...
| iOS | Detected by user agent (iPadOS by touch points). The profile is a form POST, so Safari shows its own profile prompt; `beforeunload` is suppressed for that one post. Profiles are signed by `font.dkbuilds.co` (Let's Encrypt, RSA, since 2026-09-22), not by the site's own domain: Vercel answers `/.well-known/acme-challenge/` on `*.vercel.app` and owns its DNS, so no certificate can be issued for `your-own-font.vercel.app`. The signer only has to be a publicly trusted name, and iOS shows it next to "Verified". `scripts/signing-cert.sh` issues or renews the certificate through Vercel DNS, sets `SIGN_CERT`/`SIGN_KEY`/`SIGN_CHAIN` and redeploys. Run it every ~60 days (certificates last 90). The key stays in `~/.config/your-own-font/letsencrypt` and the env var. Without the env vars the route serves unsigned | SPEC §8.2 |
| Small phones | At ≤ 400 px the wordmark hides in the Writing bar only (inks and count need the room) | The logo tile stays; every other state shows the wordmark |
...
| Starting Making | Making starts on the word completion that brings the count to 15: writing the last word, rewriting a reopened word, or the second **Next word** that keeps a stuck word as written. It never starts just because the page is already at 15/15. Someone who comes back to a finished page through the logo sees "That's both sentences." with "Tap a word to rewrite it" and one **Make my font** button, which goes straight into the lift (no beat: they have already seen the page) | The lightest honest way forward: they can change a word, or go on, and nothing moves until they choose. The button exists only in that return state, never in the normal flow |
| The beat | 1.2 s from the last word's completion (its flight into the sentence lands in about 0.7 s, then the page rests). Once the beat starts, written words can't be reopened; the logo still goes home and cancels it | A reopen racing the fold would leave Making with 14 words. The beat is short, and the logo is the way back |
| The hand-off | Writing and Making share one screen, so there is no fade between them. Writing folds away the bar, prompt, guides and footnote (300 ms), then Making mounts with the same sentences component at the same place | Nothing visibly moves at the hand-off; the sentences never re-mount under a transition |
| Which letter flies | Each glyph records the written letter it was cut from (`BuiltGlyph.from`: word, letter, and the x-offset between the word's ink and the glyph's). The flight starts from exactly the ink of the sample the font uses (the model's best, not the first written) | A letter never flies from a different sample than the one in the font |
| Flight timing | Each flight still waits on the worker's `glyph(i)` event. Letters lift every 125 ms from 200 ms after Making mounts (was 500 ms: the sentences are already on screen); the lift is `nib`, the flight `paper`, and the letter turns to metal 200 ms after it lands. The row is set once the last letter has landed | The flights overlap the old pauses, so Making takes no longer than before |
| Reduced motion (the lift) | No flights: each letter fades out of its word as it fades into its slot, paced as before (7 s ÷ 26). The rest of the sentence still fades to faint (a colour fade, not movement); after z the sentences fade out and are removed | "Flights become fades" (SPEC §9) |
| Retry and Skip | Try again after an error has no sentences to lift from, so letters lift in as they did before this change. An error before z folds the sentences away with it. Skip (second run) lands every flight at once | Nothing to lift from; Skip means skip |
